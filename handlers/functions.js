...
function onCoolDown(message, command) {
  ...
  async function change_status(client) {
    try {
      ...
      const memberss = await guild.members.fetch({force: true});
      client.user.setActivity(`${config.prefix}dm | ${guild.name} | ${memberss.filter(member => !member.user.bot).size} Members`, {
        type: "PLAYING",
      });
    } catch (e) {
      console.log(String(e.stack).bgRed)
